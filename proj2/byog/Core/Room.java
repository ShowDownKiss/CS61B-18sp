...
public class Room extends RectangleHelper {
    ...
    /** Property of a rectangle room, and p is at the bottom left. */
    ...
    private boolean containPosition(Position p) {
        return (p.x <= position.x + this.width - 1 && p.x >= position.x) &&
               (p.y <= position.y + this.height - 1 && p.y >= position.y);
    }

    public boolean isOverlap(Room r) {
        Position[] p = cornerPosition(r.position, r.width, r.height);
        for (int i = 0; i < 4; i++) {
    ...
    public Room randomRoom(Random RANDOM, TETile[][] world) {
     ...
     */
    public void removeOverLaps(List<Room> rooms){
        for (int i = 0; i < rooms.size(); i++) {
            for (int j = i + 1; j < rooms.size(); j++) {
                if (rooms.get(i).isOverlap(rooms.get(j)) || rooms.get(j).isOverlap(rooms.get(i))) {  // To get rid of one specific condition.
                    rooms.remove(j);
                    j--;
                }
            }
    ...
    public List<Room> roomGenerator(Random RANDOM, TETile[][] world) {
